module Lockable
  ...
  module InstanceMethods
    ...
    def lock(user)
      return false unless Util.is_What(user,"User") and self.class.is_permitted(user.role,"lock")
      return change_lock_status("true",user)
    end

    def unlock(user)
      return false unless Util.is_What(user,"User") and self.class.is_permitted(user.role,"unlock")
